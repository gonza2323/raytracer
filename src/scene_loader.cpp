...
void SceneLoader::load_textures(Scene& scene, const fastgltf::Asset& asset, const std::filesystem::path& base_path) {
    ...
    for (size_t i = 0; i < asset.images.size(); ++i) {
        ...
        auto process_data = [&](const std::byte* bytes, size_t size) {
            unsigned char* data = stbi_load_from_memory(reinterpret_cast<const unsigned char*>(bytes), static_cast<int>(size), &img.width, &img.height, &img.channels, 4);
            if (data) {
                img.channels = 4;
        ...
        std::visit(fastgltf::visitor {
            [&](const fastgltf::sources::URI& uri) {
                ...
                unsigned char* data = stbi_load(imagePath.string().c_str(), &img.width, &img.height, &img.channels, 4);
                if (data) {
                    img.channels = 4;
                    img.data.assign(data, data + img.width * img.height * 4);
                    stbi_image_free(data);
            ...
            [&](const fastgltf::sources::BufferView& view) {
                ...
                std::visit(fastgltf::visitor {
                    [&](const fastgltf::sources::Vector& vector) {
                        process_data(vector.bytes.data() + bufferView.byteOffset, bufferView.byteLength);
                    },
                    [&](const fastgltf::sources::ByteView& view) {
                        process_data(view.bytes.data() + bufferView.byteOffset, bufferView.byteLength);
                    },
                    [&](const fastgltf::sources::Array& array) {
                        process_data(array.bytes.data() + bufferView.byteOffset, bufferView.byteLength);
                    },
                    [](auto&) {}
                }, buffer.data);
            },
            ...
            for (auto& primitive : mesh.primitives) {
                ...
                if (normalIt != primitive.attributes.end()) {
                    ...
                    fastgltf::iterateAccessorWithIndex<glm::vec3>(asset, normalAccessor,
                        [&](glm::vec3 n, size_t) {
                            glm::vec3 worldNormal = glm::normalize(glm::vec3(transform * glm::vec4(n, 0.0f)));
                ...
                for (size_t i = 0; i + 2 < indices.size(); i += 3) {
                    ...
                    Triangle tri{};
                    tri.v0.pos = positions[i0]; tri.v1.pos = positions[i1]; tri.v2.pos = positions[i2];
                    tri.v0.normal = normals[i0]; tri.v1.normal = normals[i1]; tri.v2.normal = normals[i2];
                    
                    // If no normals were provided, compute geometric normal
                    if (glm::length(tri.v0.normal) < 0.1f) {
                        glm::vec3 n = glm::normalize(glm::cross(tri.v1.pos - tri.v0.pos, tri.v2.pos - tri.v0.pos));
