...
namespace mpi_protocol
{
    ...
    void send_tile_result(const Tile& tile, const std::vector<uint32_t>& pixels, int destination_rank)
    {
        ...
        MPI_Send(pixels.data(), static_cast<int>(pixels.size()), MPI_UINT32_T, destination_rank, kTagTileResult,
                 MPI_COMM_WORLD);
    }
    ...
    void receive_tile_result(int source_rank, Tile& tile, std::vector<uint32_t>& pixels)
    {
        ...
        MPI_Recv(pixels.data(), pixel_count, MPI_UINT32_T, source_rank, kTagTileResult, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
    }
